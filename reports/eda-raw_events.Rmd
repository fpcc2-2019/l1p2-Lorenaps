...
events = read.csv("../../events_log.csv")
...
events %>% count(page_id) %>% filter(n == 1) 
```

144676 páginas aparecem apenas uma vez nos dados

```{r}
t = events %>% count(page_id) %>% filter(n > 1) %>% count(n) %>% arrange(n)
...
```
Das páginas que aparecem mais de uma vez a maioria aparece 16 vezes. Quais os sessions ids? e data? e actions?

o que é checkin, n_results e position results?
